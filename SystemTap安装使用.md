...
3. ## 使用过程
   ...
   1. `make install`
      ...
      很奇怪必须`install`到系统中才能

   2. `qemu-trace-stap`

      1. `qemu-trace-stap list qemu-system-x86_64`

         可以列出来程序所有的探测点

      2. `qemu-trace-stap run qemu-system-x86_64 'qio*'` or `qemu-trace-stap run --pid=1732 qemu-system-x86_64 'qio*'` or `qemu-trace-stap -v run /opt/qemu/4.0.0/qemu-system-x86_64 'qio*'`
